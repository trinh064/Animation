...
//     reset when the user presses ‘r’.V
...
//     is pressed will make the red ball move twice as fast. V
//  6. The red ball only moves up/down/left/right. Change the code so that is two keys
//     are pressed simultaneously the red ball will move diagonally. V
//  7. A common pitfall in games is that the diagonal motion is actually faster than
//     horizontal motion (because you just add the vectors). Make sure your code solving Step 6 does not have that issue, and that the red ball moves diagonally at the same speed it move horizontally or vertically.V
...
//         long do you envision a pixel is in your scene).V
